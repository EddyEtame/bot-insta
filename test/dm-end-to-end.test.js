  ...
  try {
    ...
    const { processor, sent } = harness({
      ...
      aiReply: async (request) => {
        seen.push(request);
        return { reply: "Mardi à Balma : Cross training à 12h15 et MMA à 18h30. Passez quand vous voulez, sans réserver.", sourceIds: ["bc-balma-planning"] };
      },
    });
    await processor(payload("Bonjour, c'est quoi le planning du mardi à Balma ?"));
    assert.equal(sent.length, 1);
    assert.match(sent[0].text, /12h15/);
    assert.equal(seen[0].retrieval.analysis.gymIds[0], "balma");
...
test("the same question without a club asks which club instead of guessing, and never calls the model", async () => {
  ...
  try {
    ...
    assert.match(sent[0].text, /Balma, Saint-Cyprien, États-Unis, Minimes, Ramonville ou Portet/);
  } finally {
    workspace.cleanup();
  }
});

test("a question about a club with no fetched planning hands over instead of borrowing another club's", async () => {
  const { workspace, knowledge, registry } = await syncedWorkspace();
  try {
    const { processor, sent, escalations } = harness({
      ...
      knowledge,
      registry,
      aiReply: async () => ({ reply: "Mardi : MMA à 18h30.", sourceIds: ["bc-balma-planning"] }),
    });
    await processor(payload("le planning du mardi à Ramonville ?"));
...
test("an invented figure is caught after the model and turned into a handover", async () => {
  ...
  try {
    const { processor, sent, escalations } = harness({
      ...
      knowledge,
      registry,
      aiReply: async () => ({ reply: "Mardi à Balma : MMA à 20h45.", sourceIds: ["bc-balma-planning"] }),
    });
    await processor(payload("le mardi à Balma, le MMA c'est à quelle heure ?"));
...
test("the build controls pass on a synced corpus and catch a contradiction the moment it appears", async () => {
  ...
  try {
    ...
    assert.equal(clean.coverage.find((gym) => gym.gymId === "balma").hasPlanning, true);

    const fs = require("node:fs");
    const path = require("node:path");
    const existing = JSON.parse(fs.readFileSync(path.join(workspace.directory, "source", "generated", "bc-balma-planning.json"), "utf8"));
    fs.writeFileSync(path.join(workspace.directory, "source", "generated", "bc-balma-planning-copy.json"), JSON.stringify({
      metadata: { ...existing.metadata, id: "bc-balma-planning-copy", facts: [{ key: "planning_balma_mardi", value: "autre chose" }] },
      content: "Planning Balma\nMARDI : 19h Autre chose",
    }));
    fs.writeFileSync(path.join(workspace.directory, "source", "generated", "bc-balma-broken.json"), JSON.stringify({
      metadata: { ...existing.metadata, id: "bc-balma-broken", facts: [] },
      content: "Il n’y a pas de cours le dimanche à Balma.",
    }));
